...
尽管 Umami 是一款注重隐私的产品，它仍然可能被某些广告拦截器阻止。广告拦截器使用由许多不同人员维护的屏蔽列表。有些列表可能过于激进，例如阻止任何包含 `analytics` 一词的内容，或者仅仅因为其运作方式而直接屏蔽所有跟踪产品。
...
如果你使用的是像 [Next.js](https://nextjs.org) 这样的框架，可以使用它们的 [重写](https://nextjs.org/docs/pages/api-reference/next-config-js/rewrites) 功能。
...
2. 保存该文件。
3. 将文件上传到你的域名。
4. 设置 `data-host-url` 属性，以便将数据发送到正确的位置。参见 [追踪器配置](https://umami.is/docs/tracker-configuration)。
