...
如果你希望跟踪器只在特定域名上运行，你可以将它们添加到跟踪器脚本中。这是一个以逗号分隔的域名列表。每个值都会与 `window.location.hostname` 进行匹配，因此你应该仔细检查你的网站是否使用了 `www`。
如果你正在使用预发布/开发环境，这会很有帮助。
...
将事件按名称标签分组，以便进行筛选和 A/B 测试。更多详情请参见 [标签](/docs/tags)。
...
启用从访问者浏览器自动收集 [Core Web Vitals](https://web.dev/articles/vitals)。更多详情请参见 [性能](/docs/performance)。
...
尊重用户浏览器的 [禁止追踪](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/doNotTrack) 设置。
