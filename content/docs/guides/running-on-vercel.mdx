...
如果您还没有数据库，可以创建一个 Vercel Postgres 数据库集成。您也可以遵循 [在 DigitalOcean 上运行](/docs/guides/running-on-digitalocean) 指南或 [在 PlanetScale 上运行](/docs/guides/running-on-planetscale) 指南来建立数据库。您还可以查看 [托管](/docs/guides/hosting) 下的 **托管数据库** 部分。
...
3. 在仪表盘页面点击 **导入项目**，然后指定你 fork 的 GitHub 项目 URL。
...
- 数据库 URL 使用的环境变量可以在 `db/postgresql/schema.prisma` 文件中更改。
