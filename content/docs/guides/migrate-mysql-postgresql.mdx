...
作为 Umami v3 的一部分，我们将移除对 MySQL 作为受支持数据库的选项，统一采用 PostgreSQL。
此变更确保所有环境中的数据层更加一致且更具可扩展性。
...
1. 在 PostgreSQL 服务器上安装 Umami（v2.19.0）。有关安装过程的文档可在[这里](/docs/install)找到。
2. 截断下面的表。这些表将从 MySQL 导入中填充。
...
- 文件路径（/tmp/website_event.csv）位于 MySQL 服务器的文件系统上，不一定是在你的主机上。
- 确保 MySQL 有权限写入该位置（secure_file_priv 可能会限制此操作）。
