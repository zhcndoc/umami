...
- 数据库 schema 必须与最新的 v1 版本（v1.4.0）保持同步。该脚本会查询 prisma migrations 表，以确保已运行最新迁移。

### 重要提示

- 使用前请先备份目标数据库。迁移如果中断，可能会导致数据丢失。
- 对于数据量较大的用户（500万以上），迁移可能需要较长时间。请提前做好规划。
- 该脚本不会将任何事件数据迁移到 v2。
- 迁移完成后，脚本会询问你是否要删除 v1 表。
- 如果发现 `event_data` 表中已有数据，它会被重命名为 `v1_event_data`，但不会被删除。

### 故障排除

- 如果你的 `DATABASE_URL` 是 localhost，迁移时无法连接数据库，请尝试改为使用 IP 地址，例如：`127.0.0.1`。
