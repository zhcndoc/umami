...
[Koyeb](https://www.koyeb.com) 是一个面向开发者的无服务器平台，可以在全球范围内部署应用。该平台让你可以无缝运行 Docker 容器、Web 应用和 API，支持基于 git 的部署、原生自动扩展、免费 SSL、全球边缘网络，以及内置的服务网格和服务发现。
...
创建两个 Koyeb Secret，用于安全存储你的 `DATABASE_URL` 和 `APP_SECRET`。在终端中执行以下命令：

```shell
$ koyeb secret create umami-database-url
Enter your secret: postgres://<user>:<password>@<host>:<port>/<db-name>?sslmode=require

$ openssl rand -hex 32 | koyeb secret create umami-app-secret --value-from-stdin
```
...
```shell
koyeb app init umami --docker docker.umami.is/umami-software/umami:postgresql-latest --ports 3000:http --routes /:3000 --env DATABASE_URL=@umami-database-url --env APP_SECRET=@umami-app-secret
```
