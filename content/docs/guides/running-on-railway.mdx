...
[![部署到 Railway](https://railway.app/button.svg)](https://railway.app/new/template/umami-analytics)
...
1. 在 [Railway](https://railway.app/) 上创建一个与 GitHub 关联的账户。
1. 如果你愿意，在关联 GitHub 账户时可以仅授予 Railway 访问你的 Umami fork 仓库的权限。
1. 从仪表盘页面点击 **New Project > Deploy from repo**。
1. 选择你 fork 的 Umami 仓库，并选择 **master** 分支。
1. 添加第一个变量 `PORT`，将其值设为有效端口号（例如：`3000`）。点击 **Add**。
1. 添加第二个变量 `DATABASE_TYPE`，将其值设为 `postgres`。点击 **Add**。
1. 添加第三个变量 `HOSTNAME`，将其值设为 `0.0.0.0`。点击 **Add**。
1. 点击 **Deploy**。
