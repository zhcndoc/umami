...
const report = `
📊 每日报告 — ${yesterday.toDateString()}
访客数：${stats.visitors}
页面浏览量：${stats.pageviews}
访问次数：${stats.visits}
跳出率：${((stats.bounces / stats.visits) * 100).toFixed(1)}%
`;
```
...
console.log(`访客数：${thisWeek.visitors}（较上周${sign}${change}%）`);
```
