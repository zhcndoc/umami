...
- **使用 Docker compose**：使用 `docker compose` 构建你自己的 Docker 容器。
- **使用 Docker 镜像**：下载预构建的 Docker 镜像。
- **使用 HelmForge 的 Kubernetes**：使用由 HelmForge 项目维护的第三方 Helm chart 部署 Umami。请参阅 [使用 HelmForge 在 Kubernetes 上运行](/docs/guides/running-on-helmforge) 指南。
...
这将创建一个 PostgreSQL 数据库，并在 `http://localhost:3000` 上启动 Umami 应用程序。
...
使用预构建镜像时，您需要提供自己的数据库，并设置 `DATABASE_URL` 环境变量。配置选项请参见 [环境变量](/docs/environment-variables)。
