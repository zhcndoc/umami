...
| `endDate` | string | 结束日期。 |
| `model` | string | 归因模型（`first-click` \| `last-click`）。 |
...
| Parameter | Type | Description |
| --- | --- | --- |
| `startAt` | number | Unix 毫秒格式的开始日期。 |
| `endAt` | number | Unix 毫秒格式的结束日期。 |
| `timezone` | string | 时区（例如 America/Los_Angeles）。 |
| `currency` | string | 货币代码（ISO 4217）。 |
| `compare` | string | （可选）比较周期（`prev` \| `yoy`）。 |
...
返回给定日期范围内的收入统计数据，并可选对比上一周期。

**参数**

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| `startAt` | number | Unix 毫秒时间戳格式的开始日期。 |
| `endAt` | number | Unix 毫秒时间戳格式的结束日期。 |
| `timezone` | string | 时区（例如 America/Los_Angeles）。 |
| `currency` | string | 货币代码（ISO 4217）。 |
| `compare` | string | （可选）对比周期（`prev` \| `yoy`）。 |
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| `startAt` | number | 开始日期，Unix 毫秒时间戳。 |
| `endAt` | number | 结束日期，Unix 毫秒时间戳。 |
| `timezone` | string | 时区（例如 America/Los_Angeles）。 |
| `type` | string | 拆分类型（`country` \| `region` \| `referrer` \| `channel`）。 |
| `currency` | string | 货币代码（ISO 4217）。 |
...
返回一个带分页的会话列表，其中包含收入数据。

**参数**

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| `startAt` | number | Unix 毫秒时间戳的开始日期。 |
| `endAt` | number | Unix 毫秒时间戳的结束日期。 |
| `timezone` | string | 时区（例如 America/Los_Angeles）。 |
| `currency` | string | 货币代码（ISO 4217）。 |
| `page` | number | （可选）页码。 |
| `pageSize` | number | （可选）每页结果数。 |
| `search` | string | （可选）用于筛选会话的搜索词。 |
