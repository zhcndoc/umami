...
Umami 允许您将数据导入到 Umami Cloud。导入一次只能处理一个网站。
如果您有大量的网站或需要导入大量数据，请联系 [support@umami.is](mailto:support@umami.is)。

## 生成导入文件

导入文件采用 CSV 格式，包含标题和引号。列必须按以下格式和顺序提供。
只要遵循格式并填充非可选列，导入就能接受来自外部系统的数据。
下面将介绍如何将您的数据从本地托管版本的 Umami 导入到 Umami Cloud（适用于 PostgreSQL 和 MySQL）的指南。
...
您将利用 PostgreSQL 的 [\copy](https://www.postgresql.org/docs/current/sql-copy.html) 命令。
要使用该命令，您需要访问 [psql](https://www.postgresql.org/docs/current/app-psql.html) 终端。

```shell
psql -h <主机名或 IP 地址> -p <远程主机的端口号> -d <数据库> -U <用户名>
```

`\copy` 不支持多行 SQL 语句。为了绕过这个问题，您需要在使用该命令之前创建一个视图。
如果使用的模式不是 `public`，您需要在下面的语句中标识出模式（例如，`<模式>.website_event`）。
...
现在您可以使用 `\copy` psql 命令。在下面的命令中替换 `<website id>` 和 `<filepath>`。
...
您将利用 MySQL 的 [SELECT ... INTO OUTFILE](https://dev.mysql.com/doc/refman/8.0/en/select-into.html) 命令。
要使用该命令，您需要访问 [MySQL](https://dev.mysql.com/doc/refman/8.0/en/connecting.html) 客户端。

```shell
mysql --host=localhost --user=myname --password=password mydb
```

- 文件是在服务器主机上创建的，因此您必须具有 FILE 特权才能使用此语法。
- 或者，如果远程主机上安装了 MySQL 客户端软件，
  您可以使用类似 mysql -e "SELECT ..." > file_name 的客户端命令在该主机上生成文件。
- 您需要先生成视图以便重用。
...
如果返回的是 `/var/lib/mysql-files/`，则您的文件路径可以是 `/var/lib/mysql-files/my-import.csv`
...
现在您可以使用 `SELECT ... INTO OUTFILE` MySQL 命令。在下面的命令中替换 `<website id>` 和 `<filepath>`。
可以通过导航到 **网站** 并单击自托管 Umami 实例上的 **编辑** 按钮来找到 `website id`。
此命令不支持标题，因此您需要硬编码它们。
