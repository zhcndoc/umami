...
允许您将指标发送到不同于默认的 `/api/send` 的位置。这可以帮助您避免一些 [广告拦截器](/docs/guides/bypass-ad-blockers)。
...
允许您为跟踪脚本分配一个自定义名称，区别于默认的 `script.js`。这可以帮助您避免一些 [广告拦截器](/docs/guides/bypass-ad-blockers)。
