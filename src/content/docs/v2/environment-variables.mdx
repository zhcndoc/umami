...
您可以使用环境变量来配置 Umami。它们存储在与 `DATABASE_URL` 相同的 `.env` 文件中。
...
要检查客户端 IP 地址的 HTTP 头。当您在使用非标准头的代理之后时，这很有用。
...
允许您将指标发送到与默认的 `/api/send` 不同的位置。这有助于避免某些[广告拦截器](/docs/guides/bypass-ad-blockers)。
...
Umami 在收集完全匿名的遥测数据以帮助改进应用程序。如果您不想参与，可以选择禁用此功能。
...
如果您在需要绑定特定主机名或端口的环境中运行，例如 Heroku，您可以添加这些变量，并使用 `npm run start-env` 而不是 `npm start` 启动应用程序。

**IGNORE_IP = &lt;IP 地址或范围&gt;**

您可以提供逗号分隔的 IP 地址和范围列表，以排除在数据收集之外。
...
允许您为跟踪器脚本分配一个与默认值 `script.js` 不同的自定义名称。这有助于避免某些[广告拦截器](/docs/guides/bypass-ad-blockers)。

不需要使用 `.js` 扩展名。该值也可以是您选择的任何路径，例如 `/path/to/tracker`。
...
如果想在一个子目录下托管 Umami。您可能需要更新反向代理设置以正确处理 BASE_PATH 前缀。
