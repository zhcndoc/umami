...
- 派生 [Umami 仓库](https://github.com/umami-software/umami)。
- 创建一个与 GitHub 连接的 [Railway 账户](https://railway.app/)。
- 如果愿意，您可以在链接 GitHub 账户时仅给予 Railway 对您 Umami 派生库的权限。
- 在仪表板页面点击 **新项目 > 从代码库部署**。
- 选择您派生的 Umami 仓库，并选择 **master** 分支。
- 添加一个名为 `PORT` 的第二个变量，设置其值为有效的端口号（例如: `3000`）。点击 **添加**。
- 添加一个名为 `DATABASE_TYPE` 的第三个变量，设置其值为 `postgres`。点击 **添加**。
- 添加一个名为 `HOSTNAME` 的第四个变量，设置其值为 `0.0.0.0`。点击 **添加**。
- 点击 **部署**。
...
- 关闭设置侧边栏，点击 **新建**，点击 **数据库** 并选择 **添加 PostgreSQL**。
- 在仪表板上，选择您的 Umami 服务，进入 **变量** 选项卡并点击 **新变量**。
- 点击 **添加引用** 并选择来自 Postgres 数据库的 `DATABASE_URL` 变量。点击 **添加**。
