...
1. 在 Neon 上创建一个名为给定名称的项目，选择一个靠近你将托管 Umami 项目的区域。
   - 你也可以使用 Neon CLI 创建 Neon 项目：`npx neonctl projects create`。连接字符串将打印到控制台上。
...
4. 将 `DATABASE_URL` 添加到你的 `.env` 文件中：
   ...
   ```
5. 你现在应该能够检查数据库连接并更新模式（`yarn run build-db && yarn run update-db`）。
