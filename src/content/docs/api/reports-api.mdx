...
所有报表现在都可以通过请求体中的 filters 属性进行筛选。
...
- `hostname`:（字符串 | 可选）主机名。
- `tag`:（字符串 | 可选）标签名称。
- `segment`:（UUID | 可选）细分段 UUID。
- `cohort`:（UUID | 可选）群组 UUID。
...
- `type`: （字符串）报表类型（`attribution` | `breakdown` | `funnel` | `goal` | `journey` | `retention` | `revenue` | `utm`）。
- `page` : （可选数字，默认1）指定页码。
- `pageSize` : （可选数字）指定返回结果数。
...
- `type`: （字符串）报表类型（`attribution` | `breakdown` | `funnel` | `goal` | `journey` | `retention` | `revenue` | `utm`）。
- `filters`: 筛选参数。
- `startDate` : （数字）起始日期时间戳（毫秒）。
- `endDate` : （数字）结束日期时间戳（毫秒）。
- `model` : （字符串）归因模型（`firstClick` | `lastClick`）。
- `type` : （字符串数组）转化类型（`path` | `event`）。
- `step`: （字符串）转化步骤。
...
- `startDate` : （数字）起始日期时间戳（毫秒）。
- `endDate` : （数字）结束日期时间戳（毫秒）。
- `steps` : （数组，至少两个）事件类型（`path` | `event`）及转化步骤。
- `window` : （数字）漏斗步骤间视为转化的时间窗口（分钟）。
